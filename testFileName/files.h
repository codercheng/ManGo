...
*/
//declare
void clientGetFileCore(char *path, int conn_fd, char *filelen);
	...
	while(1){
		...
		if(fileinfo.isDir == 1) {
			...
			if(access(fileinfo.filename, F_OK) != 0) { /*file not exist*/
				...
				* begin getting file
				*/
				clientGetFileCore(fileinfo.filename, sockfd, fileinfo.filelen);
...
void serverSendFileInner(int sockfd, FilePathList *filelist) {
	...
	while(loop--) {
		...
		if(!tmp->isDir) {
			SendFileInner(fileinfo.filename, sockfd, atoi(fileinfo.filelen));
		}
